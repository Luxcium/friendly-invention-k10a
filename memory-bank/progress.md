...
- Created `python/` directory at project root for Python virtual environment and scripts
- Added `README.md` to `python/` documenting its purpose and relationship to `notebooks/` folder
- `notebooks/` folder (sibling to `python/`) contains Jupyter notebooks for the project
- Both folders are now tracked in the Memory Bank for project continuity
